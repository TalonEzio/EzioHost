...
namespace EzioHost.Benchmark
{
    internal sealed class ImageOpenCvUpscaler
    {
        ...
        private static Mat TensorToImageFast(Tensor<float> tensor)
        {
            ...
            // Copy vào Mat rồi merge (an toàn, dễ kiểm soát)
            ...
            var outMat = new Mat();
            Cv2.Merge([bMat, gMat, rMat], outMat); // BGR
            return outMat;
        }
        ...
        private static unsafe void Float01ToBytesAvx2(ReadOnlySpan<float> src, Span<byte> dst)
        {
            ...
            fixed (byte* pDst = dst)
            {
                for (; i <= n - 16; i += 16)
                {
                    ...
                    var i32A = Avx2.ConvertToVector256Int32(a);
                    var i32B = Avx2.ConvertToVector256Int32(b);
