...
public unsafe class FfmpegBenchmarkGemini
{
    ...
    public void IterationCleanup()
    {
        ...
        if (Directory.Exists(_currentOutputDir)) Directory.Delete(_currentOutputDir, true);
    }
    ...
    {
        var arguments =
            $"-y -i \"{VideoPath}\" -c copy -hls_time 4 -hls_list_size 0 -hls_segment_filename \"{_currentOutputDir}/segment_%05d.ts\" \"{_currentOutputDir}/playlist.m3u8\"";
        RunProcess(arguments);
    }
    ...
    public void AutoGen_RemuxToHls()
    {
        ...
        try
        {
            if (ffmpeg.avformat_open_input(&pInputFormatContext, inputFile, null, null) < 0)
                throw new Exception("Open input failed.");
            if (ffmpeg.avformat_find_stream_info(pInputFormatContext, null) < 0)
                throw new Exception("Find stream info failed.");
            ...
            for (var i = 0; i < pInputFormatContext->nb_streams; i++)
            {
                var inStream = pInputFormatContext->streams[i];
                var outStream = ffmpeg.avformat_new_stream(pOutputFormatContext, null);
            ...
            if ((pOutputFormatContext->oformat->flags & ffmpeg.AVFMT_NOFILE) == 0)
                if (ffmpeg.avio_open(&pOutputFormatContext->pb, outputFile, ffmpeg.AVIO_FLAG_WRITE) < 0)
            ...
            if (ffmpeg.avformat_write_header(pOutputFormatContext, &hlsOpts) < 0)
                throw new Exception("Write header failed.");

            var pPacket = ffmpeg.av_packet_alloc();
            while (ffmpeg.av_read_frame(pInputFormatContext, pPacket) >= 0)
            {
                var inStream = pInputFormatContext->streams[pPacket->stream_index];
                var outStream = pOutputFormatContext->streams[pPacket->stream_index];
    ...
    {
        var arguments =
            $"-y -i \"{VideoPath}\" -c:v h264_nvenc -c:a aac -hls_time 4 -hls_list_size 0 -hls_segment_filename \"{_currentOutputDir}/segment_%05d.ts\" \"{_currentOutputDir}/playlist.m3u8\"";
        RunProcess(arguments);
    }
    ...
    public void AutoGen_EncodeToHls()
    {
        static void Throw(int err, string where)
        {
            if (err < 0)
            {
                ...
                var msg = Marshal.PtrToStringAnsi((IntPtr)buf) ?? "ffmpeg error";
        ...
        var inputFile = VideoPath;
        var outputFile = Path.Combine(_currentOutputDir, "playlist.m3u8");
        var segPath = Path.Combine(_currentOutputDir, "segment_%05d.ts").Replace('\\', '/');
        var hlsTime = 4;
        ...
        try
        {
            ...
            var vInSt = ifmt->streams[vIndex];
            var aInSt = aIndex >= 0 ? ifmt->streams[aIndex] : null;

            // video decoder (threading)
            {
                var dec = ffmpeg.avcodec_find_decoder(vInSt->codecpar->codec_id);
            ...
            if (aIndex >= 0)
            {
                var dec = ffmpeg.avcodec_find_decoder(aInSt->codecpar->codec_id);
            ...
            {
                var enc = ffmpeg.avcodec_find_encoder_by_name("h264_nvenc");
                ...
                var inFps = vInSt->avg_frame_rate.num != 0
                    ? vInSt->avg_frame_rate
                    : new AVRational { num = 30, den = 1 };
                var fpsd = ffmpeg.av_q2d(inFps);
                var fps = (int)Math.Round(fpsd <= 0 ? 30.0 : fpsd);
                var gop = Math.Max(1, fps * hlsTime);

                var srcPix = vdec->pix_fmt;
                var srcOk = srcPix == AVPixelFormat.AV_PIX_FMT_YUV420P
                ...
                {
                    var fpsDen = (int)Math.Round(ffmpeg.av_q2d(venc->framerate));
                    if (fpsDen <= 0) fpsDen = 30;
                    var oneFps = new AVRational { num = 1, den = fpsDen };
            ...
            if (aIndex >= 0)
            {
                ...
                {
                    var aac = ffmpeg.avcodec_find_encoder_by_name("aac");
                    ...
                    var aFrameSize = aenc->frame_size > 0 ? aenc->frame_size : 1024;
                    ...
                    {
                        var inLayout = adec->ch_layout;
                        var outLayout = aenc->ch_layout;
            ...
            // loop
            var ipkt = ffmpeg.av_packet_alloc();
            var voutPkt = ffmpeg.av_packet_alloc();
            var aoutPkt = ffmpeg.av_packet_alloc();
            var vfrm = ffmpeg.av_frame_alloc();
            var afrm = ffmpeg.av_frame_alloc();
            ...
            while (ffmpeg.av_read_frame(ifmt, ipkt) >= 0)
            {
                ...
                if (ipkt->stream_index == vIndex)
                {
                    ...
                    while (true)
                    {
                        var r = ffmpeg.avcodec_receive_frame(vdec, vfrm);
                        ...
                        if (needScale)
                        {
                            ffmpeg.sws_scale(sws, vfrm->data, vfrm->linesize, 0, vdec->height, yuv->data,
                                yuv->linesize);
                ...
                if (!audioCopy && aIndex >= 0 && ipkt->stream_index == aIndex)
                {
                    ...
                    while (true)
                    {
                        var r = ffmpeg.avcodec_receive_frame(adec, afrm);
                        if (r == ffmpeg.AVERROR(ffmpeg.EAGAIN) || r == ffmpeg.AVERROR_EOF) break;
                        ...
                        var need = ffmpeg.av_audio_fifo_size(afifo) + conv->nb_samples;
                        Throw(ffmpeg.av_audio_fifo_realloc(afifo, need), "av_audio_fifo_realloc");

                        for (uint i = 0; i < 8; i++) planesIn[i] = conv->data[i];
                        var written = ffmpeg.av_audio_fifo_write(afifo, (void**)planesIn, conv->nb_samples);
                        ...
                        {
                            var aOutFrame = ffmpeg.av_frame_alloc();
                            ...
                            var rd = ffmpeg.av_audio_fifo_read(afifo, (void**)planesOut, aOutFrame->nb_samples);
            ...
            {
                var frameSize = aenc->frame_size > 0 ? aenc->frame_size : 1024;
                var remain = afifo != null ? ffmpeg.av_audio_fifo_size(afifo) : 0;

                while (remain > 0)
                {
                    var toSend = Math.Min(remain, frameSize);
                    ...
                    for (uint i = 0; i < 8; i++) planesOut[i] = aOutFrame->data[i];
                    var actually = ffmpeg.av_audio_fifo_read(afifo, (void**)planesOut, toSend);
