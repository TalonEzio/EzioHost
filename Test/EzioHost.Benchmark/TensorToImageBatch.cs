...
public class TensorToImageBatch
{
    ...
    private static readonly ImageEncodingParam SaveImageEncodingParam = new(ImwriteFlags.JpegQuality, 90);
    ...
    public unsafe List<DenseTensor<float>> ImageToTensor(List<Mat> images)
    {
        ...
        var batchCount = (int)Math.Ceiling(images.Count / (float)_batchSize);
        ...
        for (var b = 0; b < batchCount; b++)
        {
            ...
            var offset = b * _batchSize;
            var actualBatch = Math.Min(_batchSize, images.Count - offset);
            var totalElements = actualBatch * 3 * _height * _width;
    ...
    public unsafe List<Mat> TensorToImageWithUpscale(List<DenseTensor<float>> inputBatches)
    {
        ...
        foreach (var input in inputBatches)
        {
            ...
            var batch = upscaled.Dimensions[0];
            var channels = upscaled.Dimensions[1];
            var height = upscaled.Dimensions[2];
            var width = upscaled.Dimensions[3];
            ...
            for (var b = 0; b < batch; b++)
            {
                ...
                for (var y = 0; y < height; y++)
                {
                    ...
                    for (var x = 0; x < width; x++)
                    {
                        ...
                        var r = span[batchOffset + 0 * hw + pixelIndex];
                        var g = span[batchOffset + 1 * hw + pixelIndex];
                        var bVal = span[batchOffset + 2 * hw + pixelIndex];
                ...
                results.Add(mat);
                mat.SaveImage(Path.Combine(@"C:\Users\Talon Ezio\Downloads\Compressed", $"{Guid.NewGuid()}.jpg"),
