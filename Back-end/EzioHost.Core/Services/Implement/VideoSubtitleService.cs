    ...
    IStorageService storageService) : IVideoSubtitleService
{
        ...
        string fileName, long fileSize, Guid userId)
    {
        ...
        if (!Directory.Exists(subtitleDirectory)) Directory.CreateDirectory(subtitleDirectory);
    ...
    public async Task DeleteSubtitleAsync(Guid subtitleId)
    {
        ...
        if (subtitle == null) throw new ArgumentException("Subtitle không tồn tại", nameof(subtitleId));
    ...
    public async Task<Stream> GetSubtitleFileStreamAsync(Guid subtitleId)
    {
        ...
        var localFilePath = Path.Combine(_webRootPath, subtitle.LocalPath);
        if (!File.Exists(localFilePath)) throw new FileNotFoundException("File subtitle không tồn tại trên server");
